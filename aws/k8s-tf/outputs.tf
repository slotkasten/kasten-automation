...
output "_3_argocd_endpoint" {
  ...
  value       = var.argocd_deployment ? "https://${data.kubernetes_service.argocd_server[0].status.0.load_balancer.0.ingress[0].hostname}" : "ArgoCD not deployed"
}
...
output "_5_kasten_dashboard_cmd" {
  ...
  value       = var.argocd_deployment ? "open http://`kubectl -n kasten-io get svc gateway-ext -ojsonpath='{.status.loadBalancer.ingress[0].hostname}'`/k10/" : "ArgoCD not deployed"
}
