...
output "_3_argocd_endpoint" {
  ...
  value       = var.deployment.argocd ? (var.deployment.cert_manager ? "https://argocd.${var.creator_tag}-${terraform.workspace}.${var.domain_name} # once Gateway is up" : "https://${data.kubernetes_service.argocd_server[0].status.0.load_balancer.0.ingress[0].ip}") : "ArgoCD not deployed"
}
...
output "_5_kasten_dashboard_cmd" {
  ...
  value       = var.deployment.argocd ? (var.deployment.cert_manager ? "open https://kasten.${var.creator_tag}-${terraform.workspace}.${var.domain_name}/k10/" : "open http://`kubectl -n kasten-io get svc gateway-ext -ojsonpath='{.status.loadBalancer.ingress[0].ip}'`/k10/") : "ArgoCD not deployed"
}
